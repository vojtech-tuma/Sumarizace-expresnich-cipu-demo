...
#### + set basic variables ####
treated <- mps_nosnp$D_KID_PLIER
#treated <- mps_snp$D_KID_ITER_PLIER
original <- mps_snp$D_KID_PLIER
#random <- mps_random$D_KID_PLIER
...
dir.create(file.path("results","gene",output), recursive=TRUE)
...
#### + + bind together ####
common_ids <- intersect(treated$probeset_id, original$probeset_id)

mps_o <- original %>% filter(probeset_id %in% common_ids)
mps_o$treatment <- rep("original",nrow(mps_o))  

mps_t <- treated %>% filter(probeset_id %in% common_ids)
mps_t$treatment <- rep("treated",nrow(mps_t))

mps <- rbind(mps_o, mps_t)

mps %>% 
  unite(signal_dabg,signal,dabg) %>% 
  spread(treatment, signal_dabg) %>% 
  separate(original,c("signal_o", "dabg_o"), sep="_",convert = TRUE) %>% 
  separate(treated,c("signal_t", "dabg_t"), sep="_",convert = TRUE) -> mps_compare

mps_compare %>% filter(dabg_o<0.05 & dabg_t<0.05) %>% ggplot(aes(signal_o,signal_t)) + geom_point() + scale_x_log10() + scale_y_log10()
